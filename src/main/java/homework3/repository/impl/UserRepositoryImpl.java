...
public class UserRepositoryImpl implements UserRepository {
    ...
    public boolean addUser(User user) {
        String sql = "INSERT INTO habit_tracker.users (name, email, password, role_id, is_blocked) VALUES (?, ?, ?, ?, ?)";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            pstmt.setString(1, user.getName());
            pstmt.setString(2, user.getEmail());
            pstmt.setString(3, user.getPassword());
            pstmt.setLong(4, user.getRole().getId()); // Используем ID роли из объекта Role
    ...
    public User findByEmail(String email) {
        String sql = "SELECT u.*, r.name AS role_name FROM habit_tracker.users u JOIN habit_tracker.roles r ON u.role_id = r.id WHERE u.email = ?";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setString(1, email);
            try (ResultSet rs = pstmt.executeQuery()) {
                if (rs.next()) {
                    User user = User.builder()
                            ...
                            .role(Role.builder().id(rs.getLong("role_id")).name(rs.getString("role_name")).build()) // Создаем объект Role
    ...
    public List<User> findAll() {
        String sql = "SELECT u.*, r.name AS role_name FROM habit_tracker.users u JOIN habit_tracker.roles r ON u.role_id = r.id";
